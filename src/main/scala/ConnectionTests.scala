...
object ConnectionTests extends IOApp {
  ...
  def udpInputSender(serverHost: IpAddress, serverPort: Port, bindPort: Port): Stream[IO, Unit] =
    Stream.resource(Network[IO].openDatagramSocket()).flatMap { socket =>
      ...
      Stream.awakeEvery[IO](1.second).evalMap { _ =>
        val input = s"2,moveForward"  // You can replace with actual game inputs
  ...
  def run(args: List[String]): IO[ExitCode] = {
    ...
    val serverIp = ip"127.0.0.1"//ip"91.214.191.159"
    val tcpPort = port"9000"
    val udpInputPort = port"5555"
    val udpListenPort = port"6001"

    val streams = Stream(
      tcpClient(serverIp, tcpPort),
      udpInputSender(serverIp, udpInputPort, udpListenPort)
    ).parJoinUnbounded
